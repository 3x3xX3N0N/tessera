 ...
 * forced into `-Dtessera.native` for the duration of each test and restored afterwards, so [NativeIoTest] exercises
 * [NativeUdpIo] and [ChannelIoTest] exercises [ChannelUdpIo] no matter which task runs them (`:transport:test` runs
 * with the default `auto`; `:transport:nativeTest` with `-Dtessera.native=on`, which also puts EndpointTest itself on
...
abstract class DatapathScenarios(private val mode: String) {
    ...
    @BeforeTest fun pinDatapath() {
        ...
        if (native) assertTrue(Datapath.nativeAvailable, "tessera_native failed to load: ${NativeLib.loadError}")
    }
...
/** The same scenarios on [ChannelUdpIo], so the DatagramChannel path stays covered whatever `-Dtessera.native` says. */
