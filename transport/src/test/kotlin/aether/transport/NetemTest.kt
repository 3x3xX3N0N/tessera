...
class NetemTest {
    ...
    private fun responder(s: AetherServer, running: AtomicBoolean, response: ByteArray, beforeSend: (AetherConnection) -> Unit = {}) =
        Thread {
            while (running.get()) {
    ...
    @Test fun lostFirstResponseFromAServerThatClosesAtOnceIsRepaired() {
        net(NetemSim.Preset.TRANSCONT).use { n ->
            ...
            responder(n.s, running, "hello".toByteArray()) { sc ->
                sc.txFilter = { kind, _, _ -> kind == AetherConnection.KIND_SOURCE && dropped.getAndIncrement() == 0 }
            }
            val conn = n.c.connect(n.s.localAddress, keys.x25519Pub, keys.kemPub, "ff".toByteArray(), timeoutMs = 3_000)
            val resp = conn.receive(3_000)
    ...
    // ---------------------------------------------------------------- harness (the bench's runAether, with late/lost split)
     ...
     */
    private fun stream(conn: AetherConnection, sc: AetherConnection, n: Int, gapUs: Long, size: Int, warmup: Int, rttUs: Long, chatter: Boolean = false): StreamResult {
