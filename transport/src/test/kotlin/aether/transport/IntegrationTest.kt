...
import aether.core.RingTracer
import aether.core.TraceEvents
import aether.core.VantagePoint
...
class IntegrationTest {
    ...
    private class Pair(val s: AetherServer, val c: AetherClient, val conn: AetherConnection, val sc: AetherConnection) : AutoCloseable {
        override fun close() { c.close(); s.close() }
    }

    private fun pair(serverCfg: ConnConfig = ConnConfig(), clientCfg: ConnConfig = ConnConfig()): Pair {
        val s = server(serverCfg); val c = AetherClient(cfg = clientCfg)
    ...
    @Test fun amplificationLimitHoldsAfterMigrationUntilValidated() {
        pair(ConnConfig(creditWaitMs = 400)).use { p ->
            ...
            p.conn.txFilter = { kind, _, _ -> kind == AetherConnection.KIND_PATH }   // the client stops answering challenges
            AetherClient().use { c2 ->
                c2.adopt(p.conn)
    ...
    @Test fun connectSurvivesLostRepliesAndLostInitials() {
        server().use { s -> AetherClient().use { c ->
            fun check(conn: AetherConnection, minMs: Long, t0: Long) {
    ...
    @Test fun ptoBackoffRecoversLostFirstDataPackets() {
        pair().use { p ->
            ...
            p.sc.txFilter = { kind, _, _ ->
                when (kind) {
                    AetherConnection.KIND_SOURCE -> sources++ < 4
                    AetherConnection.KIND_REPAIR, AetherConnection.KIND_RESEND -> System.nanoTime() - start < 300_000_000L
                    else -> false
                }
            }
