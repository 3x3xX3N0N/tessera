...
| 4 | PTO schedule | `PathEstimator.ptoUs(n)` = pto, 1.5·pto, 2·pto, then doubling (2^n before the first RTT sample), capped at 2 s, never stops. The backoff counter `n` resets on **forward progress only**: firing a PTO train records the first packet number it may occupy (`PathState.armTlpProbe`), and an ack clears the counter only when the highest pn it newly acks reaches that mark (`onTlpProgress`) — the probe, or data sent after it, got through. Resetting on any ack meant that on a congested path where some traffic still returns, an ack for an old repair or re-send restarted the schedule at the base timeout with a full `PTO_TRAIN + 1` train, so the exponential never engaged; this is the F8 follow-up to gating reactive repair behind `HybridCc.engaged`. The rule is congestion-independent: with no probe outstanding (the mark is cleared) any ack resets, as before. |
