 ...
 * recovery:metrics_updated, ...) and live under "tessera:" where it is not (grants, repair, paths, handshake).
 ...
 * clock the caller supplies). Everything under `data` uses Tessera's native units: RTTs in microseconds, rates in
...
/** Event names. qlog's own names where the semantics match, "tessera:" for Tessera-specific mechanisms. */
object TraceEvents {
    ...
    const val GRANT_ISSUED = "tessera:grant_issued"
    const val REPAIR_SENT = "tessera:repair_sent"
    const val REPAIR_DECODED = "tessera:repair_decoded"
    const val PATH_ADDED = "tessera:path_added"
    const val PATH_SWITCHED = "tessera:path_switched"
    const val HANDSHAKE = "tessera:handshake"
}
...
object TraceFormat {
    ...
    /** [Wire.VERSION] as it appears in the header, e.g. "0x54530000". */
    val TESSERA_VERSION: String = "0x%08X".format(Wire.VERSION)
...
/** qlog-style header record: version/format/title, vantage point, protocol and Tessera wire version. */
private fun writeHeader(b: TraceBuilder, title: String, vantage: VantagePoint, recordSeparator: Boolean) {
    ...
    b.field("qlog_version", TraceFormat.QLOG_VERSION)
        ...
        .field("title", title)
        .field("tessera_version", TraceFormat.TESSERA_VERSION)
        .obj("trace") {
            obj("vantage_point") { field("type", vantage.wire) }
            obj("common_fields") { field("time_format", "relative"); array("protocol_type") { item("TESSERA") } }
        }
    b.endRecord()
}
...
/** [trigger] follows qlog: "time_threshold" (RACK timer, Tessera's default), "reordering_threshold", "pto_expired". */
