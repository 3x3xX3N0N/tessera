 ...
 *   key = HKDF(secret, "tessera pkt key", 32)   iv = HKDF(secret, "tessera pkt iv", 12)   hp = HKDF(secret, "tessera hp", 32)
...
class PacketKeys private constructor(val key: ByteArray, val iv: ByteArray, val hp: ByteArray, val tagLen: Int) {
    constructor(secret: ByteArray, tagLen: Int, hpKey: ByteArray? = null) :
        this(hkdf(secret, "tessera pkt key", 32), hkdf(secret, "tessera pkt iv", 12), hpKey ?: hkdf(secret, "tessera hp", 32), tagLen)
...
/**
 * Key update via the key-phase bit (RFC 9001 §6 shape). Generation n has secret_n, secret_{n+1} = HKDF(secret_n, "tessera ku"),
...
class KeyPhaseState(initialSecret: ByteArray, val tagLen: Int) {
    ...
    init {
        ...
        nextSecret = PacketKeys.hkdf(initialSecret, "tessera ku", 32); next = PacketKeys(nextSecret, tagLen, hp)
    }
    ...
    private fun rotate() {
        ...
        nextSecret = PacketKeys.hkdf(nextSecret, "tessera ku", 32); next = PacketKeys(nextSecret, tagLen, hp)
    }
}
