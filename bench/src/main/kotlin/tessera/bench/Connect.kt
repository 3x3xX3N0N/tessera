...
fun connectBench(cpuIters: Int = 2000, iters: Int = 500, netem: NetemSim? = null) {
    ...
    println(String.format(Locale.ROOT, "connect  cpu: client-build p50=%.0fus p99=%.0fus | server-accept p50=%.0fus p99=%.0fus | first-flight budget fresh=%dB resumed=%dB",
        p(client.toList(), .5), p(client.toList(), .99), p(accept.toList(), .5), p(accept.toList(), .99), ZeroRtt.MAX_FIRST_DATA, tessera.core.Resumption.MAX_FIRST_DATA))
    ...
    TesseraServer(InetSocketAddress("127.0.0.1", 0), keys, ByteArray(32) { (it * 7).toByte() }, cfg).use { s ->
        TesseraClient(cfg = cfg).use { c ->
            ...
            var first: TesseraConnection? = null
...
private fun once(c: TesseraClient, s: TesseraServer, keys: Handshake.StaticKeys, payload: ByteArray, resumed: Boolean,
