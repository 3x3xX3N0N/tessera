...
for rep in $(seq 1 "$REPS"); do
  for p in wifi-busy 5g-mmwave lte transcont; do
    ...
    L=$(./bench/bin/bench rawudp --netem "$p" --n 4000 --gapUs 500 --size 1200 --warmup 100 2>/dev/null         | sed -n 's/^rawudp *n=[0-9]* delivered=[0-9]* late=[0-9]* loss=\([0-9.]*\)%  p50=\([0-9]*\)us p90=[0-9]*us p99=\([0-9]*\)us p999=\([0-9]*\)us.*/   /p')
    set -- $L
    [ $# -eq 4 ] && echo "$rep,$p,sim,$(($1/1000)),$(($2/1000)),$(($3/1000)),$4" || echo "$rep,$p,sim,NA,NA,NA,NA"
    ./profiles.sh "$p" >/dev/null 2>&1
    L=$(./bench/bin/bench rawudp --n 4000 --gapUs 500 --size 1200 --warmup 100 2>/dev/null         | sed -n 's/^rawudp *n=[0-9]* delivered=[0-9]* late=[0-9]* loss=\([0-9.]*\)%  p50=\([0-9]*\)us p90=[0-9]*us p99=\([0-9]*\)us p999=\([0-9]*\)us.*/   /p')
    set -- $L
