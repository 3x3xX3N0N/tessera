...
/*
 * Panama FFM bindings for the tessera_native cdylib (native/rust). JDK 21 ships the FFM API under
...
/**
 * Locates and loads `tessera_native`, and creates downcall handles. Loading happens once, at first
 ...
 *
 * Search order: `-Dtessera.native.path=<file or directory>`, then the classpath resource
 * `/native/<os>-<arch>/<library>` (extracted to a content-addressed directory under
 * `java.io.tmpdir`, reused across runs).
 */
object NativeLib {
    const val PATH_PROPERTY = "tessera.native.path"
    ...
    /** `major << 16 | minor << 8 | patch` from `tessera_version()`, or -1 when unavailable. */
    val version: Int by lazy {
        if (available) downcall("tessera_version", FunctionDescriptor.of(JAVA_INT), Linker.Option.isTrivial()).invoke() as Int else -1
    }
    ...
    val packetDescSize: Long by lazy {
        if (available) downcall("tessera_packet_desc_size", FunctionDescriptor.of(JAVA_LONG)).invoke() as Long else -1L
    }

    internal fun downcall(name: String, descriptor: FunctionDescriptor, vararg options: Linker.Option): MethodHandle {
        val lookup = lookupOrNull ?: throw IllegalStateException("tessera_native is not loaded", loadError)
    ...
    private fun locate(): Path {
        ...
        val bytes = NativeLib::class.java.getResourceAsStream(resource)?.use { it.readAllBytes() }
            ?: throw UnsatisfiedLinkError("tessera_native not found on the classpath at $resource and -D$PATH_PROPERTY is not set")
        val digest = MessageDigest.getInstance("SHA-256").digest(bytes).take(8).joinToString("") { "%02x".format(it) }
        val dir = Paths.get(System.getProperty("java.io.tmpdir"), "tessera-native-$digest")
...
/**
 * `tessera_gf256_muladd`: `dst[i] ^= src[i] * c` over GF(256)/0x11D, bit-identical to
 * [tessera.core.GF256.Scalar] but SIMD (AVX2/SSSE3/NEON) and one FFI crossing per symbol.
 *
...
object Gf256Native : GF256.OffHeapKernel {
    ...
    val implementation: String = if (!available) "unavailable" else {
        when (NativeLib.downcall("tessera_gf256_impl", FunctionDescriptor.of(JAVA_INT)).invoke() as Int) {
    ...
    override fun mulAddInto(dst: MemorySegment, src: MemorySegment, len: Long, c: Int) {
        val handle = mulAddHandle ?: throw IllegalStateException("tessera_native is not available", NativeLib.loadError)
    ...
    /**
     * `ByteArray` convenience with the same contract as [tessera.core.GF256.mulAddInto]
...
/**
 * A non-blocking UDP socket driven through `tessera_udp_*`: batch send/receive over [PacketBatch]
...
class NativeUdp(bind: String = "0.0.0.0", port: Int = 0) : AutoCloseable {
    private object Handles {
        val open = NativeLib.downcall("tessera_udp_open", FunctionDescriptor.of(JAVA_LONG, ADDRESS, JAVA_SHORT))
        val close = NativeLib.downcall("tessera_udp_close", FunctionDescriptor.of(JAVA_INT, JAVA_LONG))
        val localPort = NativeLib.downcall("tessera_udp_local_port", FunctionDescriptor.of(JAVA_INT, JAVA_LONG))
        val sendBatch = NativeLib.downcall("tessera_udp_send_batch", FunctionDescriptor.of(JAVA_INT, JAVA_LONG, ADDRESS, JAVA_LONG))
        val recvBatch = NativeLib.downcall("tessera_udp_recv_batch", FunctionDescriptor.of(JAVA_INT, JAVA_LONG, ADDRESS, JAVA_LONG, JAVA_INT))
        val sendGso = NativeLib.downcall("tessera_udp_send_gso", FunctionDescriptor.of(JAVA_INT, JAVA_LONG, ADDRESS, JAVA_LONG, JAVA_SHORT, ADDRESS))
        val busyPoll = NativeLib.downcall("tessera_busy_poll", FunctionDescriptor.of(JAVA_INT, JAVA_LONG, JAVA_BOOLEAN))
    }
    ...
    init {
        ...
        if (!NativeLib.available) throw IllegalStateException("tessera_native is not available", NativeLib.loadError)
        val handle = Arena.ofConfined().use { arena ->
            Handles.open.invoke(arena.allocateUtf8String(bind), port.toShort()) as Long
        }
        if (handle < 0) throw IOException("tessera_udp_open($bind:$port) failed: ${NativeLib.errorString(-handle)}")
        fd = handle
        val p = Handles.localPort.invoke(fd) as Int
        if (p < 0) {
            Handles.close.invoke(fd)
            throw IOException("tessera_udp_local_port failed: ${NativeLib.errorString(-p.toLong())}")
        }
        localPort = p
    }
    ...
    fun sendBatch(batch: PacketBatch, count: Int = batch.capacity): Int {
        ...
        if (r < 0) throw IOException("tessera_udp_send_batch failed: ${NativeLib.errorString(-r.toLong())}")
        return r
    }
    ...
    fun recvBatch(batch: PacketBatch, timeoutMs: Int, max: Int = batch.capacity): Int {
        ...
        if (r < 0) throw IOException("tessera_udp_recv_batch failed: ${NativeLib.errorString(-r.toLong())}")
        return r
    }
    ...
    fun sendGso(batch: TxBatch, from: Int, end: Int): Int {
        ...
        if (r < 0) throw IOException("tessera_udp_send_gso failed: ${NativeLib.errorString(-r.toLong())}")
